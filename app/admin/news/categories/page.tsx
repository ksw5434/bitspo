...
export default function AdminNewsCategoriesPage() {
  ...
  const handleReorder = async (orderedItems: SortableCategoryItem[]) => {
    ...
    try {
      const res = await fetch("/api/admin/news/categories/reorder", {
        method: "PUT",
      ...
      setCategories((prev) => {
        ...
        return orderedItems
          ...
          .filter((cat): cat is NewsCategoryRecord => !!cat);
      });
    } catch (err) {
  ...
  const startEdit = (category: NewsCategoryRecord) => {
  ...
  const handleDelete = async (category: NewsCategoryRecord) => {
    if (
      !confirm(
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button asChild variant="outline" size="sm">
          <Link href="/admin/news">← 글 목록</Link>
        </Button>
      </div>

      <p className="text-sm text-muted-foreground">
        드래그로 순서를 바꾸면 사이드바·공개 뉴스 탭에 바로 반영됩니다.
      </p>
