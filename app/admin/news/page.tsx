...
import type { AdminNewsItem } from "./_components/news-edit-form";
...
export default function AdminNewsListPage() {
  ...
  const filteredNews = useMemo(() => {
    if (!activeCategory) return newsList;
    return newsList.filter((news) => newsHasCategory(news, activeCategory.id));
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setCategories(
        (categoriesJson.data ?? []).map((row: NewsCategoryRecord) =>
          normalizeNewsCategory(row),
        ),
      );
      ...
      setNewsList((newsRes.data ?? []) as unknown as AdminNewsItem[]);
    } catch (err) {
  ...
  const handleDelete = async (news: AdminNewsItem) => {
    if (!confirm(`"${news.headline}" 뉴스를 삭제할까요?`)) return;
  ...
  const newPostHref = activeCategory
    ? `/admin/news/new?category=${encodeURIComponent(activeCategory.slug)}`
    : "/admin/news/new";
            ...
            <tbody>
              {loading ? (
                ...
                filteredNews.map((news) => {
                  const categoryNames = (news.news_categories ?? [])
