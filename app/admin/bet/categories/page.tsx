...
export default function AdminBetCategoriesPage() {
  ...
  const handleReorder = async (orderedItems: SortableCategoryItem[]) => {
    ...
    try {
      const res = await fetch("/api/admin/bet/categories/reorder", {
        method: "PUT",
      ...
      setCategories((prev) => {
        ...
        return orderedItems
          ...
          .filter((cat): cat is BetCategoryRecord => !!cat);
      });
    } catch (err) {
  ...
  const startEdit = (category: BetCategoryRecord) => {
  ...
  const handleSaveEdit = async (categoryId: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/bet/categories/${categoryId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name }),
      });
  ...
  const handleDelete = async (category: BetCategoryRecord) => {
    if (
      !confirm(
    ...
    try {
      const res = await fetch(`/api/admin/bet/categories/${category.id}`, {
        method: "DELETE",
      });
      const json = await res.json();
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-lg font-semibold">Bet 카테고리 관리</h2>
        <Button asChild variant="outline" size="sm">
          <Link href="/admin/bet">← 글 목록</Link>
        </Button>
      </div>

      <p className="text-sm text-muted-foreground">
        드래그로 순서를 바꾸면 사이드바·/bet 탭에 바로 반영됩니다.
      </p>
