...
import { isCryptoNewsItem } from "@/lib/news-filters";
import {
  newsHasCryptoCategory,
  normalizeCryptoCategory,
  type CryptoCategoryRecord,
} from "@/lib/crypto-categories";
import type { AdminCryptoNewsItem } from "./_components/crypto-edit-form";

export default function AdminCryptoListPage() {
  const searchParams = useSearchParams();
  const categorySlug = searchParams.get("category");

  const [newsList, setNewsList] = useState<AdminCryptoNewsItem[]>([]);
  ...
  const cryptoNewsList = useMemo(
    () =>
      newsList.filter((news) =>
        isCryptoNewsItem(news as Parameters<typeof isCryptoNewsItem>[0]),
      ),
    [newsList],
  );

  const filteredNews = useMemo(() => {
    if (!activeCategory) return cryptoNewsList;
    return cryptoNewsList.filter((news) =>
      newsHasCryptoCategory(news, activeCategory.id),
    );
  }, [cryptoNewsList, activeCategory]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [categoriesRes, newsRes] = await Promise.all([
        fetch("/api/admin/crypto/categories"),
        supabase
          .from("news")
          .select(
            `
      ...
      setCategories(
        (categoriesJson.data ?? []).map((row: CryptoCategoryRecord) =>
          normalizeCryptoCategory(row),
        ),
      );
      ...
      setNewsList((newsRes.data ?? []) as unknown as AdminCryptoNewsItem[]);
    } catch (err) {
  ...
  const handleDelete = async (news: AdminCryptoNewsItem) => {
  ...
  const newPostHref = activeCategory
    ? `/admin/crypto/new?category=${encodeURIComponent(activeCategory.slug)}`
    : "/admin/crypto/new";
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Link
                          href={`/admin/crypto/${news.id}/edit`}
