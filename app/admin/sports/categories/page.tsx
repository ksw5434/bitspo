...
export default function AdminSportsCategoriesPage() {
  ...
  const handleReorder = async (orderedItems: SortableCategoryItem[]) => {
    ...
    try {
      const res = await fetch("/api/admin/sports/categories/reorder", {
        method: "PUT",
      ...
      setCategories((prev) => {
        ...
        return orderedItems
          ...
          .filter((cat): cat is SportsCategoryRecord => !!cat);
      });
    } catch (err) {
  ...
  const startEdit = (category: SportsCategoryRecord) => {
  ...
  const handleSaveEdit = async (categoryId: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/sports/categories/${categoryId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name }),
      });
  ...
  const handleDelete = async (category: SportsCategoryRecord) => {
    if (
      !confirm(
    ...
    try {
      const res = await fetch(`/api/admin/sports/categories/${category.id}`, {
        method: "DELETE",
      });
      const json = await res.json();
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-lg font-semibold">스포츠 카테고리 관리</h2>
        <Button asChild variant="outline" size="sm">
          <Link href="/admin/sports">← 글 목록</Link>
        </Button>
      </div>

      <p className="text-sm text-muted-foreground">
        드래그로 순서를 바꾸면 사이드바·/sports 탭에 바로 반영됩니다.
      </p>
