...
import { isSportsNewsItem } from "@/lib/news-filters";
import {
  newsHasSportsCategory,
  normalizeSportsCategory,
  type SportsCategoryRecord,
} from "@/lib/sports-categories";
import type { AdminSportsNewsItem } from "./_components/sports-edit-form";

export default function AdminSportsListPage() {
  const searchParams = useSearchParams();
  const categorySlug = searchParams.get("category");

  const [newsList, setNewsList] = useState<AdminSportsNewsItem[]>([]);
  ...
  const sportsNewsList = useMemo(
    () =>
      newsList.filter((news) =>
        isSportsNewsItem(news as Parameters<typeof isSportsNewsItem>[0]),
      ),
    [newsList],
  );

  const filteredNews = useMemo(() => {
    if (!activeCategory) return sportsNewsList;
    return sportsNewsList.filter((news) =>
      newsHasSportsCategory(news, activeCategory.id),
    );
  }, [sportsNewsList, activeCategory]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setCategories(
        (categoriesJson.data ?? []).map((row: SportsCategoryRecord) =>
          normalizeSportsCategory(row),
        ),
      );
      ...
      setNewsList((newsRes.data ?? []) as unknown as AdminSportsNewsItem[]);
    } catch (err) {
  ...
  const handleDelete = async (news: AdminSportsNewsItem) => {
  ...
  const newPostHref = activeCategory
    ? `/admin/sports/new?category=${encodeURIComponent(activeCategory.slug)}`
    : "/admin/sports/new";
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Link
                          href={`/admin/sports/${news.id}/edit`}
