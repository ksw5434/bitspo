...
function HighlightPanel({
  ...
  highlight: NonNullable<ScoreCard["highlight"]>;
}) {
  return (
    <Link
      href={highlight.href}
 ...
 * gameUrl·highlight 링크는 형제 노드로 분리 (<a> 중첩 방지)
 */
export function ScoreboardGameCard({ match, className }: ScoreboardGameCardProps) {
  ...
  const rightColumn = (
    <div className="flex w-[7.5rem] sm:w-[8.5rem] shrink-0 flex-col border-l border-border">
      ...
      {match.highlight ? (
        <HighlightPanel highlight={match.highlight} />
      ) : (
  ...
  const rootClassName = cn(
    "flex h-full min-h-[8.5rem]",
    match.gameUrl && "hover:bg-muted/20 transition-colors",
    className,
  );

  if (match.gameUrl) {
    const gameLabel = `${match.teams[0].name} vs ${match.teams[1].name}`;
    ...
    return (
      <div className={rootClassName}>
        <Link
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="flex min-w-0 flex-1 flex-col justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring rounded-sm"
          aria-label={`View game: ${gameLabel}`}
        >
          {teamsColumn}
        </Link>
        {rightColumn}
