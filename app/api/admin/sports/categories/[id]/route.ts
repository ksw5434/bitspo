...
export async function PATCH(
  ...
  try {
    ...
    if (error || !updated) {
      console.error("스포츠 카테고리 수정 오류:", error);
      return NextResponse.json(
        { error: getSportsCategoryErrorMessage(error) },
        { status: 500 },
      );
    }

    return NextResponse.json({ data: normalizeSportsCategory(updated) });
  } catch (error) {
    console.error("스포츠 카테고리 수정 예외:", error);
    return NextResponse.json(
      { error: "서버 오류가 발생했습니다." },
      { status: 500 },
    );
...
export async function DELETE(
  ...
  if (error) {
    console.error("스포츠 카테고리 삭제 오류:", error);
    return NextResponse.json(
      { error: getSportsCategoryErrorMessage(error) },
      { status: 500 },
    );
