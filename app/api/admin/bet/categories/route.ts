...
export async function GET() {
  ...
  if (error) {
    ...
    if (fallbackError) {
      console.error("Bet 카테고리 조회 오류:", fallbackError);
      return NextResponse.json(
        { error: getBetCategoryErrorMessage(fallbackError), data: [] },
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (existingError) {
      console.error("Bet 카테고리 중복 확인 오류:", existingError);
      return NextResponse.json(
        { error: getBetCategoryErrorMessage(existingError) },
        { status: 500 },
      );
    }
    ...
    if (insertError || !created) {
      console.error("Bet 카테고리 추가 오류:", insertError);
      return NextResponse.json(
        { error: getBetCategoryErrorMessage(insertError) },
        { status: 500 },
      );
    }

    return NextResponse.json(
      { data: normalizeBetCategory(created) },
      { status: 201 },
    );
  } catch (error) {
    console.error("Bet 카테고리 추가 예외:", error);
    return NextResponse.json(
      { error: "서버 오류가 발생했습니다." },
      { status: 500 },
    );
